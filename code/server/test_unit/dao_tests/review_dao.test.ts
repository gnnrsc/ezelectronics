...
//add review : test if the product exists
    ...
    // Test when product exists and reviews are successfully retrieved
    test("getProductReviews should resolve with reviews for an existing product", async () => {
        const mockGET = jest.spyOn(db, "get").mockImplementation((sql, params, callback) => {
            if (sql.includes("FROM products")) {
                callback(null, { model: "testModel" }); // Simulate product found
            }
            return {} as any;
        });
        ...
        const mockALL = jest.spyOn(db, "all").mockImplementation((sql, params, callback) => {
            callback(null, [
                { model: "testModel", user: "testUser", score: 5, date: "2024-05-27", comment: "Great product!" }
            ]);
            return {} as any;
        });
    ...
    test("getProductReviews should reject with ProductNotFoundError if the product does not exist", async () => {
        const mockGET = jest.spyOn(db, "get").mockImplementation((sql, params, callback) => {
            if (sql.includes("FROM products")) {
        ...
        await expect(reviewDAO.getProductReviews("testModel")).rejects.toThrow(ProductNotFoundError);
    ...
    // Test handling database error when checking if the product exists
    test("getProductReviews should reject if there is a database error when checking if the product exists", async () => {
        const mockGET = jest.spyOn(db, "get").mockImplementation((sql, params, callback) => {
            if (sql.includes("FROM products")) {
                callback(new Error("Database error"), null);
            }
    ...
    // Test handling database error when retrieving reviews
    test("getProductReviews should reject if there is a database error when retrieving reviews", async () => {
        const mockGET = jest.spyOn(db, "get").mockImplementation((sql, params, callback) => {
            if (sql.includes("FROM products")) {
                callback(null, { model: "testModel" }); // Simulate product found
            }
    ...
    // Test handling unexpected error in try-catch block
    test("getProductReviews should reject if there is an error in the try block", async () => {
        ...
        await expect(reviewDAO.getProductReviews("testModel")).rejects.toThrow("Unexpected Error");
