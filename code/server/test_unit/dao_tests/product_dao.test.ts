...
// available products
    ...
    test("getAvailableProducts - product exists but is not available", async () => {
        const mockALL = jest.spyOn(db, "all").mockImplementation((sql, callback) => {
            if (sql.includes("quantity > 0")) {
                callback(null, []); // Simulate no available products
            } else {
                callback(null, [testProd]); // Simulate product existence check
            }
            return {} as any;
        });
        ...
        expect(result).toEqual([]); // The product exists but is not available

        expect(mockALL).toHaveBeenCalledTimes(2);
        expect(mockALL).toHaveBeenNthCalledWith(
            1,
            "SELECT * FROM products WHERE quantity > 0 AND model = 'prodModel'",
            expect.any(Function)
        );
    ...
    test("getAvailableProducts - product does not exist", async () => {
        const mockALL = jest.spyOn(db, "all").mockImplementation((sql, callback) => {
            if (sql.includes("quantity > 0")) {
                callback(null, []); // Simulate no available products
            } else {
                callback(null, []); // Simulate product does not exist
            }
            return {} as any;
        });
    ...
    // Test when there is a database retrieval error during the existence check
    test("getAvailableProducts - handles database retrieval error for existence check", async () => {
        const mockALL = jest.spyOn(db, "all").mockImplementation((sql, callback) => {
            if (sql.includes("quantity > 0")) {
                callback(null, []); // Simulate no available products
            } else {
                callback(new Error("Retrieval Error")); // Simulate a database retrieval error
            }
            return {} as any;
        });

        const productDAO = new ProductDAO();
        await expect(productDAO.getAvailableProducts("model", null, "prodModel")).rejects.toThrow("Retrieval Error");
